...
EbNo_dB_array = 2:1:9;
BER_Teo = [0.2125, 0.1465, 0.1010, 0.0660, 0.0374, 0.0180, 0.0071, 0.0023]
Nsim_array = [871,1198, 1732, 2754, 4878, 9523, 21333, 69565];
...
for index_ = 1:EbNo_dB_array_len
    ...
    NSIMB=Nsim_array(index_); %número de símbolos (entero)
...
figure();
plot(EbNo_dB_array, BER);
plot(EbNo_dB_array, BER, EbNo_dB_array, BER_Teo);
legend('Pes Experimental','Pe Teórico')
ylabel('Pes')
xlabel('Eb/No')
title('Tasa de Errores de Símbolo para distintos valores de Eb/No (Const II)');

disp('Constelación II');
BER